...
# A run parked at a gate as the crew's OWN bare `no-mistakes axi status` renders
# it (captured on the installed CLI v1.72.0): the run status word stays
# `running`, the park shows up as `awaiting_agent`, and there is no `gate:`
# block and no findings table at all - the gate is visible only as the
