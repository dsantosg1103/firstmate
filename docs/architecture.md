...
A run head the task copy cannot bind, whether it never fetched the commit or the pipeline replayed the branch onto an advanced upstream, is attributed only when the pipeline's own runs ledger proves it is an active continuation of the submitted head, so neither a pipeline fix round nor a rebased live run ever reads as an older failed run.
